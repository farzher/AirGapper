...
const FFT_SIZE = 256;
const CYCLIC_PREFIX = 64;
const SYMBOL_SAMPLES = FFT_SIZE + CYCLIC_PREFIX;
const ACTIVE_FIRST = 8;
const ACTIVE_LAST = 88;
const CARRIER_COUNT = ACTIVE_LAST - ACTIVE_FIRST + 1;
const BITS_PER_SYMBOL = CARRIER_COUNT * 2;
const SYNC_SAMPLES = 512;
const SYNC_GAP = 32;
const PRE_GUARD = 96;
const TAIL_GUARD = 64;
...
function convolutionalDecode(codedBits, payloadBitLength) {
  const steps = payloadBitLength + TAIL_BITS;
  const stateCount = 64;
  const infinity = 1 << 28;
  let metrics = new Int32Array(stateCount);
  let next = new Int32Array(stateCount);
  ...
  for (let step = 0; step < steps; step++) {
    ...
    const receivedA = hasA ? codedBits[read++] : 0;
    const receivedB = hasB ? codedBits[read++] : 0;
    next.fill(infinity);
    for (let state = 0; state < stateCount; state++) {
      ...
      for (let input = 0; input < 2; input++) {
        ...
        if (hasA) metric += parity(register & 0x79) !== receivedA;
        if (hasB) metric += parity(register & 0x5b) !== receivedB;
...
function makeSync() {
  ...
  for (let i = 0; i < out.length; i++) {
    ...
    const edge = Math.min(1, i / 28, (out.length - 1 - i) / 28);
...
function deinterleave(slots) {
  const coded = new Uint8Array(CODED_BITS);
...
function modulateAudioPacket(payloadId, totalLen, mode, encodingId, block) {
  ...
  for (let symbol = 0; symbol < DATA_SYMBOLS; symbol++) {
    const bitBase = symbol * BITS_PER_SYMBOL;
    for (let carrier = 0; carrier < CARRIER_COUNT; carrier++) {
      a[carrier] = slots[bitBase + carrier * 2];
      b[carrier] = slots[bitBase + carrier * 2 + 1];
    }
...
function decodeQuadrant(real, imag) {
  const angle = Math.atan2(imag, real);
  if (angle >= -Math.PI / 4 && angle < Math.PI / 4) return [0, 0];
  if (angle >= Math.PI / 4 && angle < 3 * Math.PI / 4) return [0, 1];
  if (angle >= 3 * Math.PI / 4 || angle < -3 * Math.PI / 4) return [1, 1];
  return [1, 0];
}

function decodeFrame(samples, syncOffset) {
  const slots = new Uint8Array(SLOT_BITS);
  let body = syncOffset + SYNC_SAMPLES + SYNC_GAP + CYCLIC_PREFIX;
  ...
  for (let symbol = 0; symbol < DATA_SYMBOLS; symbol++, body += SYMBOL_SAMPLES) {
    ...
    for (let carrier = 0; carrier < CARRIER_COUNT; carrier++) {
      ...
      const differentialReal = cr * pr + ci * pi;
      const differentialImag = ci * pr - cr * pi;
      const [a, b] = decodeQuadrant(differentialReal, differentialImag);
      slots[write++] = a;
      slots[write++] = b;
    }
    previous = current;
  }
  const coded = deinterleave(slots);
...
class AcousticReceiver {
  ...
  process() {
    while (this.running) {
      ...
      const start = Math.max(this.scan, bestOffset - 5);
      const end = Math.min(maxCandidate, bestOffset + 5);
      ...
      const packet = decodeFrame(this.samples, refinedOffset);
      if (packet) {
        this.onPacket(packet);
        this.scan = refinedOffset + FRAME_FROM_SYNC_SAMPLES;
      } else {
        this.scan = refinedOffset + 64;
      }
      this.compact();
    }
  }
