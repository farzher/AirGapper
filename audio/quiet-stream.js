...
const ACTIVE_FIRST = 352; // 16.50 kHz
...
const MAGIC = new Uint8Array([0x41, 0x47, 0x51, 0x38]); // AGQ8
...
const MAX_INFO_BITS = Math.floor(SLOT_BITS / 2) - TAIL_BITS;
const MAX_INFO_BYTES = Math.floor(MAX_INFO_BITS / 8);
const QUIET_PACKETS_PER_FRAME = Math.max(1, Math.floor((MAX_INFO_BYTES - FRAME_HEADER_BYTES - FRAME_CRC_BYTES) / AUDIO_BLOCK_SIZE));
const FRAME_BYTES = FRAME_HEADER_BYTES + QUIET_PACKETS_PER_FRAME * AUDIO_BLOCK_SIZE + FRAME_CRC_BYTES;
const INFO_BITS = FRAME_BYTES * 8;
const CODED_BITS = (INFO_BITS + TAIL_BITS) * 2;
const FRAME_SAMPLES = FRAME_SYMBOLS * SYMBOL_SAMPLES;
const QUIET_ESTIMATED_KBPS = QUIET_PACKETS_PER_FRAME * (AUDIO_BLOCK_SIZE - RAPTOR_PACKET_ID_BYTES) /
...
function buildFrameBytes(payloadId, totalLen, mode, startOrdinal, blocks) {
  if (!Array.isArray(blocks) || blocks.length !== QUIET_PACKETS_PER_FRAME) throw new Error("Quiet frame packet count mismatch.");
  const modeCode = MODE_CODES.get(mode);
  if (modeCode === undefined || totalLen < 1 || totalLen > MAX_AUDIO_BYTES) throw new Error("Invalid Quiet transport metadata.");
...
function parseFrame(info) {
  ...
  for (let i = 0; i < QUIET_PACKETS_PER_FRAME; i++) {
    packets.push({
      payloadId,
      totalLen,
      mode,
...
function makeSlots(coded, seed) {
  ...
  for (let i = 0; i < coded.length; i++) slots[CODED_SLOTS[i]] = coded[i];
  return slots;
}
function recoverCoded(slots) {
  const coded = new Float32Array(CODED_BITS);
  for (let i = 0; i < coded.length; i++) coded[i] = slots[CODED_SLOTS[i]];
  return coded;
}
...
for (let symbol = 0; symbol < FRAME_SYMBOLS; symbol++) {
  const vector = randomPhaseVector(0x1f123bb5 ^ Math.imul(symbol + 1, 0x27d4eb2d), PILOT_COUNT);
...
function modulateQuietFrame(payloadId, totalLen, mode, startOrdinal, blocks) {
  const info = bytesToBits(buildFrameBytes(payloadId, totalLen, mode, startOrdinal, blocks));
  const coded = convolutionalEncode(info);
  const slots = makeSlots(coded, payloadId ^ Math.imul((startOrdinal >>> 0) + 1, 0x51f15e));
...
function decodeTransition(previous, current, correction, slots, write) {
  for (const carrier of DATA_POSITIONS) {
    ...
    const strength = clamp(magnitude / (nearbyPilotStrength(correction, carrier) * 0.6), 0.02, 1);
    slots[write++] = clamp(-nr / magnitude, -1, 1) * correction.reliability * strength;
  }
  return write;
}
function decodeSlots(slots) {
  return parseFrame(convolutionalDecode(recoverCoded(slots)));
}
