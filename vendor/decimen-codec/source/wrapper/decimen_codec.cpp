...
struct DecimenResult
{
	...
	int modules{};
	// Exact module-center coordinates used by zxing-cpp's successful SampleGrid.
	// Null on ordinary readFull/readFullY calls; populated by readFullYWithSampleMaps.
	emscripten::val sampleMap = emscripten::val::null();
};
...
static emscripten::val toFloat32Array(const std::vector<PointF>& points)
{
	if (points.empty())
		return emscripten::val::null();
	std::vector<float> xy(points.size() * 2);
	for (size_t i = 0; i < points.size(); ++i) {
		xy[i * 2] = float(points[i].x);
		xy[i * 2 + 1] = float(points[i].y);
	}
	thread_local const emscripten::val Float32Array = emscripten::val::global("Float32Array");
	return Float32Array.new_(emscripten::typed_memory_view(xy.size(), xy.data()));
}
...
// Robust acquisition that exposes the exact distortion-corrected module centers
// generated by SampleQR/SampleGrid. This is intentionally separate from the
// compatibility readFullY entry point: the receiver uses it only to seed the
// persistent locked decoder, then stops paying finder/detector cost.
std::vector<DecimenResult> readFullYWithSampleMaps(int bufferPtr, int width, int height, int stride,
                                                    bool tryHarder, int maxSymbols)
{
	try {
		ImageView iv(reinterpret_cast<uint8_t*>(bufferPtr), width, height, ImageFormat::Lum, stride, 1);
		...
		auto finderPatterns = QRCode::FindFinderPatterns(*bits, tryHarder);
		auto finderSets = QRCode::GenerateFinderPatternSets(finderPatterns);
		std::vector<ConcentricPattern> used;
		std::vector<DecimenResult> results;
		if (maxSymbols > 0)
			results.reserve(maxSymbols);

		auto alreadyUsed = [&](const ConcentricPattern& candidate) {
			return std::any_of(used.begin(), used.end(), [&](const ConcentricPattern& prior) {
				return distance(candidate, prior) < std::max(2.0, std::min(candidate.size, prior.size) * 0.5);
			});
		};

		for (const auto& fp : finderSets) {
			if (alreadyUsed(fp.bl) || alreadyUsed(fp.tl) || alreadyUsed(fp.tr))
				continue;
			for (auto&& detected : QRCode::SampleQR(*bits, fp)) {
				...
				if (!decoded.isValid())
					continue;
				results.push_back({true, "", toUint8Array(decoded.content().bytes), detected.position(),
							   detected.bits().width(), toFloat32Array(detected.samplePoints())});
				used.push_back(fp.bl);
				used.push_back(fp.tl);
				used.push_back(fp.tr);
				break;
			}
			if (maxSymbols > 0 && int(results.size()) >= maxSymbols)
				break;
		}
		return results;
	} catch (const std::exception& e) {
...
static void addBatchMetrics(DecimenBatchMetrics& dst, const DecimenBatchMetrics& src)
{
	...
	dst.binarizeMs += src.binarizeMs;
	dst.exactMapAttempts += src.exactMapAttempts;
									 ...
									 bool allowRS, DecimenBatchMetrics& measured)
{
	...
	for (int y = 0; y < dim; ++y)
		for (int x = 0; x < dim; ++x) {
			const auto& cached = track.samples[size_t(y) * dim + x];
			PointF p{cached.x + track.dx, cached.y + track.dy};
			if (!imageBits.isIn(p)) {
				++measured.outOfFrameMisses;
				measured.samplingMs += emscripten_get_now() - sampleStarted;
				return {};
			}
			track.sampled.set(x, y, imageBits.get(p));
		}
	measured.samples += dim * dim;
	measured.samplingMs += emscripten_get_now() - sampleStarted;
...
EMSCRIPTEN_BINDINGS(DecimenCodec)
{
	...
	value_object<DecimenResult>("DecimenResult")
		...
		.field("sampleMap", &DecimenResult::sampleMap);
	...
	function("readFullYWithSampleMaps", &readFullYWithSampleMaps);
