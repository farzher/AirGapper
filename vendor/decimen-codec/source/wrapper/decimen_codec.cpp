                                         ...
                                         PointF outputOffset = PointF{0, 0})
{
    ...
    // Every element is overwritten exactly once. Avoid value-initializing the
...
// Avoid a projective division per sampled module whenever the current tracked
...
constexpr float GUIDED_STABLE_ADAPT_MAX_MODULE = 2.05f;
...
{
    result.dataOnlyAttempted = false;
    ...
    *metrics = {};
    ...
    try {
        ...
        for (int i = 0; i < trackCount; ++i) {
            ...
            // Every calibrated slot may use warped Stable-RS immediately while
