...
class GridLattice {
  ...
  accept(detection, frameWidth, frameHeight) {
    ...
    if (this.locked && this.candidate) {
      // A decoded packet proves the existing lattice is still alive. Native workers
      // refine each QR on every frame, so do not solve the global 8x8 homography once
      // per QR. Coalesce global fitting to roughly camera-frame rate instead.
