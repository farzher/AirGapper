...
function verify(pixelCount) {
  ...
  const result = compactRgbaGreenInPlace(rgba.buffer, pixelCount);
  if (!(result instanceof Uint8Array) || result.length !== pixelCount)
    throw new Error(`bad compact result for ${pixelCount} pixels`);
  for (let pixel = 0; pixel < pixelCount; pixel++) {
...
if (compactRgbaGreenInPlace(null, 10) !== null) throw new Error("invalid buffer was accepted");
if (compactRgbaGreenInPlace(new ArrayBuffer(8), 3) !== null) throw new Error("undersized buffer was accepted");
