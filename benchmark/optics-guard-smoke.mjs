...
// then exposure/ISO without repeating exposureMode.
{
  ...
  assert.deepEqual(
    opticsGuardDecision({ state, patch: { exposureTime: 34, iso: 150 }, current: manual, now: 1250, auto: true }),
    { allow: true, reason: "freeze first QR" }
  );
  assert.deepEqual(
    opticsGuardDecision({ state, patch: { exposureTime: 15, iso: 300 }, current: manual, now: 1250, auto: true }),
