...
try{
  ...
  if(report.labSmoke?.workerBuild!=='v35i'||!report.labSmoke?.versionText.includes('v35i')||!report.labSmoke.versionText.includes('hardware lab')||!report.labSmoke?.autoSender||!report.labSmoke?.autoReceiver||!report.labSmoke?.exportJson||!report.labSmoke?.exportCsv||!report.labSmoke?.staleResultsCleared||!report.labSmoke?.quickSweepText.includes('12-condition binary/M1')||report.labSmoke?.quickDwell!=='2'||!report.labSmoke.receiverVisible||!report.labSmoke.senderHidden)failures.push('automated hardware lab receiver UI did not initialize');
