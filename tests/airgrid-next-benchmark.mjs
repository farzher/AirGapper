...
try{
  ...
  const expression=String.raw`(async()=>{
    ...
    async function render({profile='M1',pitch=3,cols=3,rows=2,frame=77}={}){const r=await A.request(renderer,'render',{config:{profile,pitch,cols,rows,frame,session:0x13579bdf}}),c=document.createElement('canvas');c.width=r.width;c.height=r.height;c.getContext('2d',{alpha:false}).drawImage(r.bitmap,0,0);r.bitmap.close();return{canvas:c,meta:r};}
    ...
    function compact(name,r){return{name,detector:r.detectorSize,fiducials:r.fiducials,candidates:r.candidates,tracked:r.tracked,complete:r.completeTiles,headers:r.headerOk,payloads:r.payloadOk,frames:r.frames,ber:r.metrics.rawBer,miChip:r.metrics.mutualInformationPerChip,verified:r.metrics.verifiedUniqueBytes,timings:r.timings,tiles:r.tiles.map(t=>({x:t.header.tileX,y:t.header.tileY,frame:t.header.frame,ok:t.payloadOk,rawErrors:t.rawErrors,eccIterations:t.eccIterations}))};}
    ...
    return{format:A.format,seed:'0x6d2b79f5',results,profileResults,tracking:{first:compact('tracking-acquire',trackA),second:compact('tracking-follow',trackB)},selftest:st.result};
  })()`;
