...
replacement = '''    auto sampleByte = [&](const std::vector<uint32_t>& plan, size_t firstBit,
    ...
    // The five-point low-density vote is deliberately robust but expensive:
    // every ambiguous bit adds four more bilinear Y samples. When no-RS has
    // already learned that this optical regime needs QR error correction, read
    // each bit once and mark a codeword containing an ambiguous center sample as
    // an RS erasure. If erasure-assisted RS+CRC fails, only those ambiguous
    // codewords are re-read with sampleByte() above, reconstructing the exact
    // pre-existing robust matrix before the ordinary RS fallback.
    ...
    auto runRs = [&](const ByteArray& source, const ByteArray* erasureFlags) -> DecoderResult {
        ...
        if (erasureFlags) {
            ...
            if (erasureBlocks.size() != blocks.size())
                return {};
        }
        int dataBytes = 0;
        ...
        for (size_t blockIndex = 0; blockIndex < blocks.size(); ++blockIndex) {
            ...
            if (erasureFlags) {
                ...
                for (int i = 0; i < int(flags.size()); ++i)
                    if (flags[i]) erasures.push_back(i);
                // Preserve at least two parity symbols for detection/correction
                // of mistakes outside the explicit low-confidence set.
                if (int(erasures.size()) > eccCount - 2)
                    return {};
            }
...
'''
if anchor not in s:
    raise SystemExit('sampleByte anchor missing')
...
new = '''        auto& noRsGate = guidedStableNoRsGate();
        const bool tryNoRsFirst = guidedTryNoRsFirst(noRsGate);
        // Center-only Stable-RS is already one pixel read/bit, so erasure
        // sampling cannot save camera reads there. The win is the robust
        // bilinear/ambiguity-voted path used by affine/perspective handheld QR.
...
new = '''    for (int codeword = firstParityCodeword; codeword < totalCodewords; ++codeword) {
    ...
    // First decode the cheap center-only sample with explicit erasures. Require
    // the same AirGapper CRC here before trusting it; otherwise reconstruct only
    // the suspect bytes with the original five-point vote and continue exactly
    // as v263 did.
...
'''
if old not in s:
    raise SystemExit('RS tail anchor missing')
s = s.replace(old, new, 1)
p.write_text(s)

v = Path('vendor/decimen-codec/source/VERSION')
if v.read_text().strip() != '0.1.56':
