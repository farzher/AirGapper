...
# Exhausting the QR-specific ladder hands control to AE, but AE must remain
# observable so a genuinely good camera-selected setting can become HOLD.
replace_after(
...
# Existing seed/memory measurement failure paths must return to observable AE,
# not an infinite retry deadline that can never promote a later good AE state.
for marker in [
...
if 'APP_VERSION = "0.5.455"' not in version:
    raise SystemExit("expected v0.5.455 before bump")
version_path.write_text(version.replace('APP_VERSION = "0.5.455"', 'APP_VERSION = "0.5.456"', 1))
