...
# Six workers see each physical slot only intermittently. Once a low-density
# slot has accumulated high-confidence evidence that Stable-RS loses to Sparse,
# learn that decision sooner and re-probe often enough to recover after a pose
# change. Deferred failures are only confirmed when Sparse CRC-proves the frame.
replace_once(
    cpp,
...
# Choose the shared residual from the best optical anchor instead of whichever
# track happens to appear first. A 50-QR wall makes slot ordering a terrible
# proxy for quality: first slots are commonly near an edge. Restrict references
# to repair-mask-authorized indices so temporal/pressure fencing remains real,
# then prefer proven Stable-RS maps and central geometry.
...
print("staged v0.5.316: central proven wall reference and faster per-slot recovery")
