...
# The old controller inherited normal-camera assumptions.  At 30 fps it called
# 10 ms (30% of a frame) motion-safe even though the successful hardware traces
# sit near 3.3 ms.  AirGapper is an animated emissive binary target: use a short
# integration first, then let gain supply only as much brightness as decoding
# actually needs.
...
# Keep FocusController's hardware-AE acquisition bias consistent with the owner.
rep('receive/focus-controller.js', 'const AUTO_QR_EV_BIAS = -0.7;', 'const AUTO_QR_EV_BIAS = -0.8;')

# Candidate measurements must be attributed to the camera frame that was
# CAPTURED under that setting, not to wall-clock completion time.  Under motion
# Guided jobs can finish 50-250ms later; the old global before/after counters
# could therefore award an old exposure's outputs to the next exposure.
...
# Never restore a stale manual exposure before the first QR.  Cold acquisition
# belongs to hardware AE+AF; remembered settings are only seeds after a verified
# lattice exists and can provide matched geometry for comparison.
sub('receive/main.js',
...
# Replace the fixed-delay/global-counter ISO tournament with a short,
# source-attributed robustness-boundary search.  Every camera write discards two
# actual source frames, every sample is pose-gated, movement invalidates rather
# than scores a candidate, and the selected darkest near-best candidate is
# independently confirmed once before it can become memory.
sub('receive/main.js',
...
# The old pre-lock randomized exposure race was the source of visible brightness
# thrash.  Acquisition is now deterministic hardware AE/AF only.  Also remove
# the now-unreachable memory-validation branch from the steady-state controller.
sub('receive/main.js',
...
# The developer button should exercise exactly the production controller, not a
# separate seven-second optimizer with different objectives and failure modes.
...
if 'trackedOutputSymbols - before.outputs' in receive:
    raise SystemExit('old wall-clock output attribution survived')
if 'const autoOpticsCompletionSamples = [];' not in receive or 'sourceSequence: auditMode.sourceSequence' not in receive:
    raise SystemExit('source-attributed auto optics evidence missing')
