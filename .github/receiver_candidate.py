...
def rep(path, old, new, count=1):
    ...
    if old not in s: raise SystemExit(f"missing anchor {path}: {old[:80]!r}")
...
('index.html','id="record-corpus" type="button">Record</button>','id="record-corpus" type="button">Record raw</button>')]: rep(path, old, new)

ag = Path('receive/agcap.js').read_text()
anchor = '''function u32(value) {
...
if anchor not in ag: raise SystemExit('missing agcap helper')
ag = ag.replace(anchor, insert, 1)
...
if anchor not in ag: raise SystemExit('missing frame anchor')
ag = ag.replace(anchor, insert, 1)
...
if n != 1: raise SystemExit(f'recorder call count {n}')
...
insert = '''async function captureDirectSourceScan(source) {
...
function cancelScanCapture() {
  clearTimeout(scanCaptureTimer);'''
if anchor not in main: raise SystemExit('missing cancel capture')
main = main.replace(anchor,insert,1)
anchor = '''  receiverFrameWidth = vw;
  receiverFrameHeight = vh;
  const now = receiverNow();'''
insert = '''  receiverFrameWidth = vw;
  receiverFrameHeight = vh;
  if (captureNextScan && !pendingScanCapture && source.videoFrame && !source.image) await captureDirectSourceScan(source);
  const now = receiverNow();'''
if anchor not in main: raise SystemExit('missing capture frame top')
main = main.replace(anchor,insert,1)
if main.count('source.image || captureNextScan ||') < 2: raise SystemExit('missing capture blockers')
...
if ids < 4: raise SystemExit(f'capture id replacements {ids}')
...
if old not in main: raise SystemExit('missing camera replay')
main = main.replace(old,new,1)
...
insert = '''try {
  ...
  const rawYRoundTrip = await page.evaluate(async () => {
    ...
    const metadataPreserved = recorded.visibleRect?.x === 40 && recorded.visibleRect?.width === 560 && recorded.codedWidth === 640 && recorded.displayWidth === 560;
  ...
  await page.evaluate(() => document.getElementById("home-button").click());'''
if anchor not in runner: raise SystemExit('missing benchmark anchor')
runner = runner.replace(anchor,insert,1)
Path('benchmark/offline-runner.mjs').write_text(runner)

for path, needle in [('receive/agcap.js','header.pixelFormat === "Y8" && header.formatVersion === 5'),('receive/main.js','Captured exact camera Y frame'),('receive/main.js','rawYReplay ? corpus.videoFrame(index)'),('benchmark/offline-runner.mjs','AIRGAPPER_AGCAP_RAW_Y_PASS')]:
    if needle not in Path(path).read_text(): raise SystemExit(f'missing invariant {path}: {needle}')
