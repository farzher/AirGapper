...
if text.count(old3) != 1: raise SystemExit("strict toggle block not found")
...
if text.count(old4) != 1: raise SystemExit("submitReceiverJob header not found")
...
if text.count(old5) != 1: raise SystemExit("noteDecodeCompleted header not found")
text = text.replace(old5, new5)
old6 = '''  if (completion.nativeMetrics) {\n    lastNativeMetrics = { ...completion.nativeMetrics, frameCopyMs: completion.frameCopyMs };\n    hotPathAudit.trackedJobs++;'''
new6 = '''  if (completion.nativeMetrics) {\n    lastNativeMetrics = { ...completion.nativeMetrics, frameCopyMs: completion.frameCopyMs };\n  }\n  if (auditThisCompletion && completion.nativeMetrics) {\n    hotPathAudit.trackedJobs++;'''
if text.count(old6) != 1: raise SystemExit("native audit accumulation start not found")
