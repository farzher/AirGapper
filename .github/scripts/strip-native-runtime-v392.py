...
# Receiver camera policy is receiver-owned; platform.js stays genuinely shared.
replace_one(
...
# Retired Android Camera2 bridge imports.
...
# Remove backend persistence and all native-camera-only process state.
sub_one(
    r'const CAMERA_BACKEND_KEY = "airgapper:apk-camera-backend:v1";[\s\S]*?let nativePreviewImage;\n',
    '',
    "native backend state",
)

# Remove every Camera2-specific helper/function. These functions are unreachable
# now that APK, PWA and browser all use getUserMedia + the same worker path.
for name in [
    "currentNativeTrack",
    "nativeModeLabel",
    "nativePreviewRotation",
    "syncNativePreviewAspect",
    "selectedNativeCamera",
    "nativeAutoMode",
    "populateNativeCameraModes",
    "refreshNativeCameraDevices",
    "stopNativeReceiverSource",
    "drawNativePreview",
    "drawNativeV2Preview",
    "nativeV2PreviewBounds",
    "nativeV2TrackModuleSize",
    "nativeV2WallMotion",
    "nativeV2SourceFrame",
    "submitNativeV2Job",
    "completeNativeV2Job",
    "startNativeV2Receiver",
    "startNativeReceiver",
    "nativeSourceFrame",
]:
...
sub_one(
    r'if \(nativeCamera2\) void refreshNativeCameraDevices\(\)\.catch\(\(error\) => \{[\s\S]*?\}\);\n',
    '',
    "initial native refresh",
)

# Settings changes always restart the browser camera; there is no backend mode.
sub_one(
...
sub_one(
    r'cameraBackend\?\.addEventListener\("change", \(\) => \{[\s\S]*?\n\}\);\ncameraDevice\?\.addEventListener',
    'cameraDevice?.addEventListener',
    "backend listener",
)
replace_one(
...
replace_one(
    ...
    "devicechange native guard",
)
sub_one(
    r'screen\.orientation\?\.addEventListener\?\.\("change", \(\) => \{ if \(nativeCamera2\) \{ syncNativePreviewAspect\(\); queueOverlayDraw\(\); \} \}\);\n',
    '',
    "native orientation listener",
)

# Stop/pause only own browser MediaStream + worker state now.
s = s.replace('  stopNativeReceiverSource();\n', '')

# Browser camera workers are always the normal adaptive pool.
...
# Native Camera2 had its own job submission bridge. Browser frames always use
# DecodeWorkerPool and transferable VideoFrame/ImageData ownership.
replace_one(
...
# Camera-memory identity is browser track identity only.
...
replace_one(
    ...
    "native settings epoch wait",
)
replace_one('let latestNativeSettingsEpoch = 0;\n', '', "native settings epoch state")

# Browser video is already display-oriented; overlay coordinates use it directly.
...
# Diagnostics report the browser track directly.
sub_one(
...
# Native raw-Y/native-v2 sources no longer exist. Preserve the browser
# TrackProcessor VideoFrame route and buffered replay route only.
sub_one(
    r'  if \(captureNextScan && !pendingScanCapture && source\.nativeV2 && !source\.image\) \{[\s\S]*?\n  \}\n  if \(captureNextScan && !pendingScanCapture && source\.nativeY && !source\.image\) \{[\s\S]*?\n  \}\n  if \(source\.nativeY\) drawNativePreview\(source\);\n',
    '',
...
replace_one(
    ...
    "native buffered lane guard",
)
replace_one('    if (source.nativeY || source.nativeV2) break;\n', '', "native lane break")
replace_one('  if (source.nativeV2) return;\n', '', "native optimizer skip")

# mappedDirectTrackedFrame had two Camera2-only buffer branches before the
# browser VideoFrame branch. Remove those blocks by cutting to the first browser
# VideoFrame guard; no decoding math changes.
sub_one(
...
# Completely dead timeline/event telemetry: no consumer reads the stored data.
sub_one(
...
# Guard the cleanup: the retired Android camera implementation must not leave
# any executable/reference hooks behind. "nativeMetrics" etc are WASM decoder
# terminology and intentionally remain.
for token in [
